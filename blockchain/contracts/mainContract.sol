...
contract DecentralizedForum {
    ...
    // On-chain input length caps (bytes). Kept private (no public getter) to
    // save bytecode; the values are asserted directly in the tests.
    ...
    struct RemoveModeratorProposal {
        ...
        bool exists;
        string reason;
        // Snapshotted at creation so a later change in moderator count can't
        // move the goalposts, and a deadline so a stale vote can't linger.
    ...
    // person may still be a moderator via the creator or active-moderator
    // role; ModeratorRemoved fires in the same tx only if they lost everything.
        ...
        postMustExist(postId)
    {
        ...
        // A ban on this specific community is the more specific, actionable
        // reason - check it before the generic anti-spam gate below, since a
        // ban also strips membership and can drop userCommunityCount to 0.
        if (isBanned[communityId][msg.sender]) {
            revert UserBannedFromCommunity();
        }

        // Anti-spam: only accounts that actually belong to a community (a
        // "real" participant) may vote. Brand-new wallets that never joined
        // anything cannot brigade scores.
    ...
    function _tryExecuteRemoveModeratorProposal(uint256 proposalId) private returns (bool) {
        ...
        // Losing the appointed role is distinct from losing all moderator
        // access: a double-role moderator (also creator or active) keeps
        // their status.
    ...
    // Cheap anti-spam signal for other contracts (e.g. ForumModeration's
    // report gate): has this address joined at least one community, ever
    // (net of leaves/bans)? Deliberately not the raw counter - keeps this
    // contract's public surface answering "who is this user" questions only.
    function hasJoinedAnyCommunity(address user) external view returns (bool) {
        return userCommunityCount[user] > 0;
    }
